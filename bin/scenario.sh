...
for id in $lst_nodes; do
	...
	./bas.tk --whatwho --verbose=-1 --auto --ident=$id --iconic --begin=HLG \

		--autosend --dest=HLG --delay=1000 < /tmp/in_${id}_bas > /tmp/out_${id}_bas &
	...
	./hlg.tk --whatwho --verbose=-1 --auto --ident=$id -geometry +$j+$j --begin=BAS \
